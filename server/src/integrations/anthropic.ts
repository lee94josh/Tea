...
export async function writeArticleAnthropic(
  ...
  const prompt = [
    ...
    '- A one- or two-sentence intro that states plainly what this is and why it is',
    '  worth knowing. No scene-setting, no throat-clearing.',
    '- Then 4–7 bullet points, each a self-contained fact the reader keeps. Open each',
    '  bullet with a short bold lead-in and an em-dash, then the fact. Example:',
    '  "**Origin —** opened in 2019 by chef ..." Vary the lead-ins; don\'t force a',
    '  template. One crisp sentence per bullet, two at most.',
    '- ~180–280 words total. Dense and skimmable beats long and smooth.',
    '',
    ...
    '',
    'Return the bullets in body_paragraphs: index 0 is the plain intro, each remaining',
    'entry is ONE bullet starting with "• ". Respond with ONLY this JSON (no prose',
    'around it): { "headline": "...", "dek": "...", "body_paragraphs": ["..."] }',
  ]
    .filter(Boolean)
    .join('\n');
